...
Scenario1.continuous <- function(size,ncov,seed){
  set.seed(seed)
  ...
  plot(A,Y)
  abline(h=S)
  traininfo = list(X=X,A=A,Y=Y,mu=mu,S=S,propensity=propensity/mean(propensity),opt=c,label=label)
  return(traininfo)
}
Scenario1.ordinal <- function(size,ncov,seed,K=10){
  ...
  plot(A,Y)
  abline(h=S)
  traininfo = list(X=X,A=A,Y=Y,mu=mu,S=S,propensity=rep(1,size),opt=c)
  return(traininfo)
}
Scenario2.continuous <- function(size,ncov,seed){
  set.seed(seed)
  X = matrix(runif(size*ncov,-1,1),ncol=ncov)
  #A = runif(size,aL,aU)
  A = rtruncnorm(size,a=aL,b=aU,mean=mean(aL+aU),sd=0.5)
...
Scenario2.ordinal <- function(size,ncov,seed,K=10){
  ...
  plot(A,Y)
  abline(h=S)
  traininfo = list(X=X,A=A,Y=Y,mu=mu,S=S,propensity=rep(1,size),opt=c,label=label)
  return(traininfo)
}
Scenario3.continuous <- function(size,ncov,seed){
  set.seed(seed)
  X = matrix(runif(size*ncov,-1,1),ncol=ncov)
  c=2*(0.15*X[,1]+0.15*X[,2]+0.15*X[,3]+0.15*X[,4])
  A = runif(size,aL,aU)
  ...
  plot(A,Y)
  abline(h=S)
  traininfo = list(X=X,A=A,Y=Y,mu=mu,S=S,propensity=rep(1,size),opt_L=pmax(c-len,aL),opt_R=pmin(c+len,aU),label=label)
  return(traininfo)
}
Scenario3.ordinal <- function(size,ncov,seed,K=20){
  ...
  plot(A,Y)
  abline(h=S)
  traininfo = list(X=X,A=A,Y=Y,mu=mu,S=S,propensity=rep(1,size),K=K,opt_L=pmax(c-len,0),opt_R=pmin(c+len,K),label=label)
  return(traininfo)
}
Scenario4.continuous <- function(size,ncov,seed){
  set.seed(seed)
  X = matrix(runif(size*ncov,-1,1),ncol=ncov)
  A = runif(size,aL,aU)
